...
def decision (image_imread, box_coord ) :
    ...
    turn_left = False
    output = ""
    if x_min_supp - 10 < x_min : 
        turn_right = True
        turn_left = False
        output = " TURN RIGHT -- "
    elif x_min < x_min_supp + 10 :
        turn_left = True
        trun_right = False
        output = " TURN LEFT -- "
    elif x_min_supp - 10 < x_min < x_min_supp + 10 :
        turn_left = False
        trun_right = False
        output = " NO CHANGE -- "
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        # temp 
                        #xyxy_temp = torch.tensor([163.0, 70.0, 477.0, 410.0])
                        coord_centre_box, finale_decision = decision (im0, [ele.tolist() for ele in xyxy] ) 
            ...
            if bool_enter : 
                ...
                x_max = xyxy[2]
                y_max = xyxy[3]            
                print("image's shape : ", im0.shape)
                print(xyxy)
                print("Contours's coordinates ---- ", x_min, y_min, x_max, y_max)
                cv2.imwrite("/home/majji/Documents/project_github/person detection yolov5/yolov5-master/anass_roi.jpg", im0)
                bool_enter = False
